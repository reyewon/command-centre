...
// ─── Credit card limits from KV ─────────────────────────────────────────
async function getCreditLimits(): Promise<Record<string, number>> {
  if (!KV_REST_API_URL || !KV_REST_API_TOKEN) return {};
  try {
    const res = await fetch(`${KV_REST_API_URL}/get/rcc:credit-card-limits`, {
      headers: { Authorization: `Bearer ${KV_REST_API_TOKEN}` },
      cache: 'no-store',
    });
    const data = await res.json();
...
export async function GET() {
  const [starling, trading212, manualBalances, creditLimits] = await Promise.all([
  ...
  for (const card of creditCards) {
    accounts.push({
      ...
      creditLimit: creditLimits[card.id] ?? null,
      currency: 'GBP',
